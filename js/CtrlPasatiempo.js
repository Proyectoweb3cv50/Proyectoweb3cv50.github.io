...
import { muestraPasatiempos } from "./navegacion.js";
import { tieneRol } from "./seguridad.js";

const daoPasatiempo = getFirestore().collection("Pasatiempo");
...
async function guarda(evt) {
  try {
    ...
    await daoPasatiempo.doc(id).set(modelo);
    muestraPasatiempos();
  } catch (e) {
    muestraError(e);
  }
...
async function elimina() {
  try {
    if (confirm("Confirmar la " + "eliminación")) {
      await daoPasatiempo.doc(id).delete();
      muestraPasatiempos();
    }
  } catch (e) {
    muestraError(e);
